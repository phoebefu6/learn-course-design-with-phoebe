...
(function () {
  ...
  if (crumb && mastheadWrap) {
    var mx = crumb.textContent.match(/Designer session (\d+) of 6/);
    var m = mx || crumb.textContent.match(/Delivery session (\d+) of 6/);
    if (m) {
      var current = parseInt(m[1], 10);
      var pages = mx
        ? ["a1-objectives.html", "a2-backwards-design.html", "a3-the-arc.html", "a4-activities.html", "a5-assessment-and-feedback.html", "a6-materials-and-the-room.html"]
        : ["b1-needs-analysis.html", "b2-adult-motivation.html", "b3-running-the-room.html", "b4-transfer.html", "b5-measuring-change.html", "b6-selling-it-internally.html"];
...
(function () {
  "use strict";
  var PASSPORT_KEY = "lwp-passport:course-design";
  var TOTAL_SESSIONS = 12;
  ...
  var FNS = {
    ...
    contrast: function (s) {
      ...
      if (hx.length < 2) return { error: "Give me two hex colours, for example  #FFFFFF #6E6055" };
      function lum(h) {
        h = h.replace("#", "");
  ...
  var cmA = crumbEl && crumbEl.textContent.match(/Designer session (\d+) of 6/);
  var cmB = crumbEl && crumbEl.textContent.match(/Delivery session (\d+) of 6/);
  var here = cmA ? parseInt(cmA[1], 10) : cmB ? parseInt(cmB[1], 10) + 6 : null;
  ...
  if (strip) {
    ...
    var note = el("p", "pp-note", ppCount(got2) === 0
      ...
      : ppCount(got2) >= TOTAL_SESSIONS
        ? "Twelve of twelve. Both tracks finished - you can design the learning and prove it changed something."
